...
import { getChangeForPeriod, getPercentChange } from '@app/content/Info/utils/infoDataHelpers'
import { ProtocolData } from '@app/store/info/types'
import { getDeltaTimestamps } from '@app/content/Info/utils/infoQueryHelpers'
import { useBlocksFromTimestamps } from '@app/content/Info/hooks/useBlocksFromTimestamps'

interface LegionFactory {
...
const formatLegionFactoryResponse = (rawLegionFactory?: LegionFactory) => {
  if (rawLegionFactory) {
    return {
      totalTransactions: parseFloat(rawLegionFactory.totalTransactions),
      totalVolumeUSD: parseFloat(rawLegionFactory.totalVolumeUSD),
      totalLiquidityUSD: parseFloat(rawLegionFactory.totalLiquidityUSD),
    }
  }
  return null
}
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatLegionFactoryResponse(data?.legionFactories?.[0])
      const overviewData24 = formatLegionFactoryResponse(data24?.legionFactories?.[0])
      const overviewData48 = formatLegionFactoryResponse(data48?.legionFactories?.[0])
