...
import { State, SerializedFarm, DeserializedFarmUserData, DeserializedFarm, DeserializedFarmsState } from '../typed'
...
export const usePriceLegionBusd = (): BigNumber => {
  const legionBnbFarm = useFarmFromPid(1)
  
  const legionPriceBusdAsString = legionBnbFarm.tokenPriceBusd
  
  const legionPriceBusd = useMemo(() => {
    return new BigNumber(legionPriceBusdAsString)
  }, [legionPriceBusdAsString])

  return legionPriceBusd
}
