...
import { Currency, currencyEquals, ETHER, Percent, WETH } from '@aulyaaryansyah/legionswap-sdk-mainnet'
import { BigNumber } from '@ethersproject/bignumber'
import { useTranslation } from '@app/context/Localization'
import { CurrencyLogo, DoubleCurrencyLogo } from '@app/components/Currency'
import { ROUTER_ADDRESS } from '@app/config/constants'
import useActiveWeb3React from '@app/hooks/useActiveWeb3React'
import { useCurrency } from '@app/hooks/Tokens'
import { usePairContract } from '@app/hooks/useContract'
...
import { useApproveCallback, ApprovalState } from '@app/hooks/useApproveCallback'
import { useBurnActionHandlers, useDerivedBurnInfo, useBurnState } from '@app/store/burn/hooks'
import { Field } from '@app/store/burn/slice'
import { useGasPrice, useUserSlippageTolerance } from '@app/store/user/hooks'
import {
  Box,
  Button,
  Divider,
...
import { RowBetween, RowFixed } from '@app/components/Layout/Row'
...
export default function RemoveLiquidityContent({
  ...
  const handleSelectCurrencyA = useCallback(
    (currency: Currency) => {
      if (currencyIdB && parseCurrencyId(currency) === currencyIdB) {
        router.push(`/remove/${parseCurrencyId(currency)}/${currencyIdA}`)
      } else {
        router.push(`/remove/${parseCurrencyId(currency)}/${currencyIdB}`)
      }
    },
  ...
  const handleSelectCurrencyB = useCallback(
    (currency: Currency) => {
      if (currencyIdA && parseCurrencyId(currency) === currencyIdA) {
        router.push(`/remove/${currencyIdB}/${parseCurrencyId(currency)}`)
      } else {
        router.push(`/remove/${currencyIdA}/${parseCurrencyId(currency)}`)
      }
    },
    [currencyIdA, currencyIdB, router]
  )
  ...
  return (
    <VStack flex="1 0 100%" align="stretch">
      <VStack align="stretch" backgroundColor="legion.dark" p="4" rounded="3xl" spacing="4">
        ...
        <VStack align="stretch" spacing="4">
          <AutoColumn gap="20px">
            <RowBetween>
              <Text>{t('Amount')}</Text>
              <Button colorScheme="primary" variant="link" scale="sm" onClick={() => setShowDetailed(!showDetailed)}>
                {showDetailed ? t('Simple') : t('Detailed')}
              </Button>
            </RowBetween>
            {!showDetailed && (
              <Box borderColor="brand.bg.1" borderWidth="1px" rounded="2xl" p="4">
                <Text fontSize="40px" fontWeight="bold" mb="16px" style={{ lineHeight: 1 }}>
          ...
          {!showDetailed && (
            <>
              <ColumnCenter>
                <ArrowDownIcon width="24px" my="16px" height="24px" />
              </ColumnCenter>
              <AutoColumn gap="10px">
                <Text fontWeight="bold" color="legion.secondary" fontSize="12px" textTransform="uppercase">
                  {t('You will receive')}
                </Text>
                <NoSSR>
                  <Box bg="brand.bg.1" p="4" rounded="2xl" shadow="inner">
                    <Flex justifyContent="space-between" mb="8px">
                      <Flex align="center">
                        <CurrencyLogo currency={currencyA} />
                        <Text fontSize="sm" id="remove-liquidity-tokena-symbol" ml="4px">
                          {currencyA?.symbol}
                        </Text>
                      </Flex>
                      <Text fontSize="sm">{formattedAmounts[Field.CURRENCY_A] || '-'}</Text>
                    </Flex>
                    <Flex justifyContent="space-between">
                      <Flex align="center">
                        <CurrencyLogo currency={currencyB} />
                        <Text fontSize="sm" id="remove-liquidity-tokenb-symbol" ml="4px">
                          {currencyB?.symbol}
                        </Text>
                      </Flex>
                      <Text fontSize="sm">{formattedAmounts[Field.CURRENCY_B] || '-'}</Text>
                    </Flex>
          ...
          {showDetailed && (
            <Box my="16px">
              <CardToken
                ...
                onValueChange={onLiquidityInput}
                onMax={() => {
                  onUserInput(Field.LIQUIDITY_PERCENT, '100')
                }}
                showMaxButton={!atMaxAmount}
                disableCurrencySelect
                selectedToken={pair?.liquidityToken}
                pair={pair}
                id="liquidity-amount"
                setSelected={() => null}
              />
              ...
              <CardToken
                ...
                onValueChange={onCurrencyAInput}
                onMax={() => onUserInput(Field.LIQUIDITY_PERCENT, '100')}
                showMaxButton={!atMaxAmount}
                selectedToken={currencyA}
                label={t('Output')}
                setSelected={handleSelectCurrencyA}
              ...
              <CardToken
                ...
                onValueChange={onCurrencyBInput}
                onMax={() => onUserInput(Field.LIQUIDITY_PERCENT, '100')}
                showMaxButton={!atMaxAmount}
                selectedToken={currencyB}
                label={t('Output')}
                setSelected={handleSelectCurrencyB}
          ...
          {pair && (
            <AutoColumn gap="10px" style={{ marginTop: '16px' }}>
              ...
              <Box bg="brand.bg.1" p="4" rounded="2xl" shadow="inner">
                <Flex justifyContent="space-between">
                  <Text fontSize="sm">1 {currencyA?.symbol} =</Text>
                  <Text fontSize="sm">
                    {tokenA ? pair.priceOf(tokenA).toSignificant(6) : '-'} {currencyB?.symbol}
                  </Text>
                </Flex>
                <Flex justifyContent="space-between">
                  <Text fontSize="sm">1 {currencyB?.symbol} =</Text>
                  <Text fontSize="sm">
          ...
          <Box position="relative" mt="16px">
            {!account ? (
              ...
              <RowBetween>
                <Button
                  colorScheme={approval === ApprovalState.APPROVED || signatureData !== null ? 'success' : 'primary'}
